"""
Core Algorithmic Engine & Cryptographic / Biological Logic for SyntheticControl-Arm: Propensity Score & G-Computation Observational Matcher.
    ...
    def evaluate_primary_parameter(cls, value: float) -> Optional[Dict[str, Any]]:
        if value > cls.PRIMARY_BOUND:
            return {
                ...
                "details": f"Parameter value ({value:.3f}) exceeds operational threshold ({cls.PRIMARY_BOUND:.1f}) under FDA Real-World Evidence (RWE) Guidance.",
    ...
    def audit_specification_conformance(cls, descriptor: str, attributes: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if any(flag in desc_upper for flag in ["VIOLATION", "DISCORDANT", "ANOMALY", "MUTANT", "LEAK"]):
            return {
                ...
                "details": f"Telemetry status flag '{descriptor}' violates FDA Real-World Evidence (RWE) Guidance conformance matrix.",
