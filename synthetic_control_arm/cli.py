"""
Command-Line Interface for SyntheticControl-Arm: Propensity Score & G-Computation Observational Matcher.
"""
import argparse
import csv
import json
...
from .agents import SyntheticControlCoordinator

coordinator = SyntheticControlCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="clinical-trial-synthetic-control-agent", description="SyntheticControl-Arm: Propensity Score & G-Computation Observational Matcher")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  SYNTHETICCONTROL-ARM: PROPENSITY SCORE & G-COMPUTATION OBSERVATIONAL MATCHER")
        print(f"  Domain: Biomedical Scientific Discovery & Multi-Omics | Standard: FDA Real-World Evidence (RWE) Guidance")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting SyntheticControl-Arm: Propensity Score & G-Computation Observational Matcher on http://{args.host}:{args.port}")
